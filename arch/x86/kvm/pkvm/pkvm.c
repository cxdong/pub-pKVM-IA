...
{
	BUG_ON(!addr || !PAGE_ALIGNED(addr) || !PAGE_ALIGNED(size));
...
static inline bool pkvm_event_allowed(struct kvm_vcpu *vcpu, bool for_injection)
{
	return !for_injection || (!kvm_event_needs_reinjection(vcpu) &&
...
{
	if (pkvm_event_allowed(vcpu, for_injection))
					   ...
					   union pkvm_hc_data *in, union pkvm_hc_data *out)
{
	...
	if (!vcpu || WARN_ON_ONCE(hvcpu != this_cpu_read(host_vcpu)))
		return -EINVAL;
