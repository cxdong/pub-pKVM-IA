...
#undef GEN
	...
	/*
	 * Is it possible for the dest to be the following?
	 * srso_alias_safe_ret
	 * srso_safe_ret
	 * srso_untrain_ret
	...
	if (dest == srso_alias_untrain_ret)
		return srso_alias_untrain_ret__pkvm - srso_alias_untrain_ret;
...
static void patch_pkvm(s32 *start, s32 *end)
{
	s32 *s;
	...
	for (s = start; s < end; s++) {
		...
		if (insn.opcode.bytes[0] == JMP32_INSN_OPCODE ||
		    insn.opcode.bytes[0] == CALL_INSN_OPCODE) {
			s32 disp = insn.immediate.value;
			void *dest = addr + insn.length + disp;
			uintptr_t offset = get_pkvm_offset(dest);

			if (WARN_ON_ONCE(offset == 0)) {
				pr_err_ratelimited("%pS (%px) not found in pkvm's symboles\n",
						   dest, dest);
				continue;
			}
			apply_reloc(4, &disp, offset);
			text_poke_early(addr + insn_offset_immediate(&insn), &disp, 4);
		}
	}
}
