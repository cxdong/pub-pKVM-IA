...
static void copy_host_vm_trace(void **buf, unsigned long *size)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (*size >= sizeof(struct perf_data)) {
			copy_vmexit_perf_data(*buf, perf);
			*buf += sizeof(struct perf_data);
...
static int __copy_guest_vm_trace(struct pkvm_vm *vm, void *param)
{
	...
	for (i = 0; i < vm->kvm.created_vcpus; i++) {
		...
		copy_vmexit_perf_data(arg->buf, &vm->vcpus[i]->perf);
		arg->buf += sizeof(struct perf_data);
...
static void copy_guest_vm_trace(int vm_handle, void *buf, unsigned long size)
{
	struct copy_arg arg = {
		.vm_handle = vm_handle,
		.buf = buf,
...
int pkvm_dump_vmexit_trace(phys_addr_t phys, unsigned long size, int vm_handle)
{
	...
	void *buf = __pkvm_va(phys);

	if (ret)
		return ret;

	if (vm_handle == PKVM_HOST_VM_HANDLE)
		copy_host_vm_trace(&buf, &buf_size);

	copy_guest_vm_trace(vm_handle, buf, buf_size);
